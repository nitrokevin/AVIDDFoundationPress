...
// Customizer preview webpack configuration
const webpackCustomizer = {
  config: {
    ...baseWebpackConfig,
    externals: {
      jquery: "jQuery",
  ...
  build() {
    return gulp
      .src(PATHS.customizerEntries || "src/assets/js/customizer-preview.js")
      .pipe(named())
      .pipe(webpackStream(webpackCustomizer.config, webpack2))
      .pipe(
        $.if(
          PRODUCTION,
          $.uglify().on("error", (e) => {
            log.error("[uglify-customizer:error]", e);
          }),
        ),
      )
      .pipe(
  ...
  watch() {
    const watchConfig = Object.assign({}, webpackCustomizer.config, {
      watch: true,
      devtool: "inline-source-map",
    });
    ...
    return gulp
      ...
      .pipe(
        webpackStream(
          watchConfig,
          webpack2,
          webpackCustomizer.changeHandler,
        ).on("error", (err) => {
          log.error(
            "[webpack-customizer:error]",
            err.toString({
...
gulp.task("webpack-customizer:build", webpackCustomizer.build);
gulp.task("webpack-customizer:watch", webpackCustomizer.watch);
...
function watch() {
  ...
  gulp
    .watch("src/assets/scss/**/*.scss", gulp.series(updateThemeJson, styles))
    .on("change", (path) =>
  ...
  gulp
    .watch(
      ...
      gulp.series("webpack-editor:build", reload),
    )
    .on("change", (path) =>
      log("Editor file " + colors.bold(colors.magenta(path)) + " changed."),
    )
    .on("unlink", (path) =>
      log("Editor file " + colors.bold(colors.magenta(path)) + " was removed."),
    );

  gulp
    .watch(
      "src/assets/js/customizer-preview.js",
      gulp.series("webpack-customizer:build", reload),
    )
    .on("change", (path) =>
      log("Customizer file " + colors.bold(colors.magenta(path)) + " changed."),
    )
    .on("unlink", (path) =>
      log(
        "Customizer file " +
          colors.bold(colors.magenta(path)) +
          " was removed.",
      ),
    );
}
...
gulp.task(
  ...
  gulp.series(
    ...
    gulp.parallel(
      "webpack:watch",
      "webpack-editor:watch",
      "webpack-customizer:watch",
      watch,
    ),
  ),
);

// Package: build then zip
