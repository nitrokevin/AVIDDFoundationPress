<?php

/**

 */
$id = get_query_var('block_id', 'tabs-' . wp_rand());
?>
<?php if (have_rows('repeater_content_tab')) {
	$counter = 0;
	$tab_bar_background_color = get_field('tab_bar_background_color'); ?>
	<ul class="tabs <?php echo esc_attr($tab_bar_background_color); ?>" data-tabs id="<?php echo esc_attr($id); ?>">
		<?php while (have_rows('repeater_content_tab')) {
			the_row();
			$tab_heading = get_sub_field('tab_heading');
			$tab_background_color = get_sub_field('tab_background_color');

			$counter++;
		?>
			<li class="tabs-title <?php if ($counter == 1) {
										echo "is-active";
									} ?> <?php echo esc_attr($tab_bar_background_color); ?>">
				<a href="#tab<?php echo absint($counter); ?>-<?php echo esc_attr($id); ?>" aria-selected="true">
					<?php echo esc_html($tab_heading); ?>
				</a>
			</li>
		<?php } ?>
	</ul>
	<?php $counter = 0; ?>
	<div class="tabs-content <?php echo esc_attr($tab_bar_background_color); ?>" data-tabs-content="<?php echo esc_attr($id); ?>">
		<?php while (have_rows('repeater_content_tab')) {
			the_row();
			$tab_content = get_sub_field('tab_content');
			$counter++; ?>
			<div class="tabs-panel  <?php if ($counter == 1) {
										echo "is-active";
									} ?>" id="tab<?php echo $counter ?>-<?php echo esc_attr($id); ?>">
				<?php echo wp_kses_post($tab_content); ?>
			</div>
		<?php } ?>
	</div>
<?php } ?>